...
{
    [Serializable]
    class ContextualMemoryEdgeDetectionAlgorithm : EdgeDetectionAlgorithm
    {
        ...
        public ContextualMemoryEdgeDetectionAlgorithm(ISet<ColorChannelEnum> colorChannels, int longestContextLength = 5, int tableSizeBits = 22, int numberOfRays = 8)
        {
            ...
            outputProbabilitySquashFactor = confidenceSquashFactor / numberOfContexts / floatUnit;

            sse = new SecondarySymbolEstimationStretchInput(256, 32, 0.0005f);
        ...
        private void updateProbability(int positionX, int positionY, float groundTruthProbability, float probability, float feebackProbability, int computedIndex)
        {
            ...
            for (int i = 0; i < numberOfContexts; i++)
            {
                int tableValue = contextTableMap[i][computedIndexes[computedIndex, i] & tableSizeMasks[i]];
                ...
                contextTableMap[i][computedIndexes[computedIndex, i] & tableSizeMasks[i]] = (short)tableValue;
            }
        }
