...
{
    [Serializable]
    class ContextualMemoryEdgeDetectionAlgorithmNoCollisions : EdgeDetectionAlgorithm
    {
        ...
        short[][] contextTableMap;
        int[] tableSizeMasks;

        sbyte[][] contextTableHit;
        ...
        public ContextualMemoryEdgeDetectionAlgorithmNoCollisions(ISet<ColorChannelEnum> colorChannels, int longestContextLength = 5, int tableSizeBits = 22, int numberOfRays = 8)
        {
            ...
            contextTableMap = new short[numberOfContexts][];
            contextTableHit = new sbyte[numberOfContexts][];

            for (int i = 0; i < numberOfContexts; i++)
            {
                contextTableMap[i] = new short[1 << tableBitSizes[i]];
                contextTableHit[i] = new sbyte[1 << tableBitSizes[i]];
            }
        ...
        private void computeProbabilityForPosition(int positionX, int positionY, int computedIndex, out float probability, out float feebackProbability, out float outputProbability)
        {
            ...
            for (int i = 0; i < numberOfContexts; i++)
            {
                int index = computedIndexes[computedIndex, i] & tableSizeMasks[i];
                if (contextTableHit[i][index] == computedIndexes[computedIndex, i] >> 24)
                {
                    sum += contextTableMap[i][index];
                    numberOfHits++;
                }
            }
        ...
        private void updateProbability(int positionX, int positionY, float groundTruthProbability, float probability, float feebackProbability, int computedIndex)
        {
            ...
            {
                int index = computedIndexes[computedIndex, i] & tableSizeMasks[i];
                contextTableHit[i][index] = (sbyte)(computedIndexes[computedIndex, i] >> 24);
                int tableValue = contextTableMap[i][index];
#if useEntropyLoss
                ...
                contextTableMap[i][index] = (short)tableValue;
            }
        }
